...
export default function ReservationsPage() {
  ...
  // Al sentar: cambia el estado de la reserva Y envía un ticket a cocina si hay platos y no se ha enviado
  const handleSeat = (reservation) => {
    seatReservation(reservation.id)
    if (reservation.items && reservation.items.length > 0 && !reservation.ticketSent) {
      addTicket({
      ...
      updateReservation(reservation.id, { ticketSent: true })
    }
  }
